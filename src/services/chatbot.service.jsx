...
export const getCollectionActive = () => async (dispatch) => {
  try {
    dispatch(fetchCollectionStart());
    const response = await api.get(`/api/v1/collections/active`);
    console.log({ response });
    dispatch(fetchCollectionSuccess(response.data.data));
    return response.data;
  } catch (error) {
    const errorMessage =
      error.response?.data?.message || "Không thể lấy bộ  dữ liệu";
